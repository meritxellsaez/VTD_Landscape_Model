function ABC_SMC_Algorithm_OLCM_VTD_landscape_v5Refit_Gauss_AbsDistance(mutantstofit,parfitnumbers,EpT,T,Tmax,Nmax,Numberofparticleseachjob,PreviousData,namenewdata,CovarianceMatricesParticles,NoClassifiedOption,weightsdistances,streamnum,jobnum)
try
    ...
    % To solve critical points equations. Defining them globally saves time
    syms x y
    ...
    %Initial parameters:
    initpar = [0,   1,   1,0,0.2,-2,0.5,0.3, 0.6,0.5,-0.5,-1.6,0.,0., -9.4,-4.5,0,-1.6,0.,0.,20, 0.01];
    ...
    load('DataProp150420_ClusteredDataWithThd_v4fit.mat','DataToFit')
    ...
    if T==0
        ...
        while (N<Numberofparticleseachjob)&&(i<MaxStep)
            ...
            while (parami<=numel(parfitnumbers))
                ...
                if feval(constraintshandle,paramaux) %Check condition before going to next parameter
                    parami = parami+1;
                end
            ...
            if priorOK
                ...
                [y0, InitialCondition] = VTD_Landscape_Model_Fitting_v5Refit_InitialCondition(x,y,paramaux,nsimulations,dt);
            end
            
            %Simulate dynamical system if correct parameters and initial condition, and check threshold
    ...
    if (T>0)&&(T<Tmax)
        ...
        while (N<Numberofparticleseachjob)&&(i<MaxStep)
            ...
            parami = 1;  %Initialise loop variable
            ...
            while (parami<=nparfit)&&(priorOK)  %Only if priorOK is not 0 and parami<npar the while loop will stop
                ...
                if feval(constraintshandle,paramaux) %Check that constrains are satisfied
                    parami = parami+1;
                end
    ...
    fileID = fopen(['DetailedErrorSaddle_',num2str(jobnum, '%02d'),'.txt'],'a');
    fprintf(fileID,getReport(ME,'extended'));
    fclose(fileID);
    save(['paramFatesM5v3_',num2str(jobnum, '%02d')], '-v7.3');
end

%ABC_SMC_Algorithm_OLCM_VTD_landscape_v5Refit_Gauss_AbsDistance.m (c) by Elena
%Camacho Aguilar and Meritxell Saez Cornellana

%ABC_SMC_Algorithm_OLCM_VTD_landscape_v5Refit_Gauss_AbsDistance.m is licensed under a
