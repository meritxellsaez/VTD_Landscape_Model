function pvalue=VTD_Landscape_Model_Fitting_v5Refit_Priors(paramnumber)
...
elseif paramnumber == 17
    %p103        
        pvalue = random('Normal',0,0.03);        
elseif paramnumber == 18
    %p104
        pvalue = random('Normal',-1.4,0.03);
...
elseif paramnumber == 21
    %M
         pvalue = random('Normal',18,2.5);
elseif paramnumber == 22
    %D
        pvalue = random('Uniform',1.e-5,1.5e-2);   
end

%VTD_Landscape_Model_Fitting_v5Refit_Priors.m (c) by Elena
%Camacho Aguilar and Meritxell Saez Cornellana

%VTD_Landscape_Model_Fitting_v5Refit_Priors.m is licensed under a
