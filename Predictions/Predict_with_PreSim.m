...
load('Reference_Set_Clusteringv5.mat','NewData', 'MixedGaussian');

Particles = NewData(1:10000,:);
...
%Mutants to simulate:
mutantstofit=[12,13]; %Choose mutants to simulate
...
p = parcluster('local');
parfor i=1:numPart
    paramaux = Particles(i,:); %Change to predict for a subset of the parameters
...
save('Predictions_Initial.mat','NewFates','errorparams', 'mutantstofit','-v7.3')




%Predict_with_PreSim.m (c) by Elena
%Camacho Aguilar and Meritxell Saez Cornellana

%Predict_with_PreSim.m is licensed under a
