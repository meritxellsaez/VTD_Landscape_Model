...
%%Choose one Dataset
DataSet = '18_08_14'; %%options: '18_08_14' or '15_4_20' or '15_4_20_FOXC2' or '15_4_20_OTX2' or '18_10_25'
...
switch DataSet
    case '18_08_14'
        ...
        %indxsPlot = [1 5 3 9 7 11];
        indxsPlot = [1 3 9 5 11 7 6];%[1 3 9 5 11 7]; %%Colors for histogram plots
        OrderProteins=1:5;
    case '17_3_20'
        NumIdxs = 11;
        DataSetSave = '170320';
    ...
    case '15_4_20'
        ...
        dimsClust = 1:5; %%Change to cluster using only a subset of the dimensions
        %indxsPlot = [1 5 3 9 7 11];
        indxsPlot = [1 3 9 5 11 7 6 13];%[1 3 9 5 11 7]; %%Colors for histogram plots
        OrderProteins=[4 1 5 3 2];
    case '15_4_20_FOXC2'
        NumIdxs = 1;
...
for DataIdx = 1:NumIdxs %%Load Data from all experimental conditions
    [Data{DataIdx}, Proteins, NameData{DataIdx}, NameDays{DataIdx}, DataSetSave, SaveName] =...
...
elseif strcmp(DataSet, '15_4_20_FOXC2')
    ...
    Proteins{1}='FOXC2';
elseif strcmp(DataSet, '15_4_20_OTX2')
...
elseif strcmp(DataSet, '18_10_25')
...
elseif strcmp(DataSet, '12_5_20_OTX2')
    DataIdxsAll={1,[1,3],[1,3], [1,3,4], [1,3,4], [1,3,4], [1,3,4]};
    NumClustAll=[1, 3, 2, 4, 4,5,5];
    InitmuAll={1.e+3 *[7.9491    2.3310    4.8007    0.8788],...
        1.e+4*[0.7637    0.3679    0.2253    0.1145; 0.5209    0.2480    0.2760    0.0811; 1.3209    0.3338    0.5691    0.1204],...
        1.e+4*[0.3107    0.3895    0.1292    0.0989; 1.0076    0.2552    0.4457    0.1054],...
        1.e+4*[0.2911    0.3429    0.0956    0.1339; 0.5624    0.3315    0.3857    0.2246; 0.3096    0.7715    0.1789    0.1592; 1.7349    0.4434    0.9242    0.2422],...
        1.e+4*[0.6088    0.2669    0.3807    0.4306; 1.8170    0.2645    0.9130    0.2721; 0.2096    0.2235    0.0775    0.1130; 0.2127    0.6591    0.1318    0.1522],...
        1.e+4*[0.1984    0.1893    0.0763    0.1020; 0.3998    0.1934    0.6783    0.6430; 1.0458    0.2188    1.0225    0.2644; 1.8886    0.3098    1.1779    0.4068; 0.1829    0.7566    0.1925    0.1473],...
        1.e+4*[0.2208    0.2046    0.0762    0.1306; 0.2076    0.9088    0.3245    0.2276; 0.8521    0.1907    0.9345    0.2392; 1.5791    0.2588    1.1210    0.3798; 0.3108    0.1885    0.6302    0.6635]};
    indxsPlotLandsAll={2,[9,8,2],[9,2],[15,13,9,6],[13,6,15,9],[15,13,6,6,9],[15,9,6,6,13]};
end
gmfitAll=cell(1,max(NumDaysAll));

%% Perform clustering day by day
...
for DaysToUse=1:max(NumDaysAll)
    ...
    DataAnalise = zeros(numel(DataIdxs)*NumDaysUsed*NumSamples, NumProt+1); %Put all Data in a matrix
    ...
    gmfitAll{DaysToUse}=gmfit; %save obtained gmm
end
