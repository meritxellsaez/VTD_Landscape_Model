function ABC_SMC_Algorithm_OLCM_VTD_landscape_v5_Gauss_AbsDistance(mutantstofit,parfitnumbers,EpT,T,Tmax,Nmax,Numberofparticleseachjob,PreviousData,namenewdata,CovarianceMatricesParticles,NoClassifiedOption,weightsdistances,streamnum,jobnum)
try
    ...
    load('DataProp180814_ClusteredDataWithThd_v4.mat','DataToFit')
    ...
    if isfile([namenewdata,'_',num2str(jobnum, '%02d'),'.mat']) %Load partial results if execution was interrupted
        load([namenewdata,'_',num2str(jobnum, '%02d'),'.mat'],'NewData','NewFates', 'substreamnum');
        N= size(NewData,1);
        if N==Numberofparticleseachjob
            return;
        end
        substreamnum = substreamnum+500;
        cmrg.Substream = substreamnum;
    else
        N = 0;
        NewData = [];
    ...
    if T==0
        ...
        while (N<Numberofparticleseachjob)&&(i<MaxStep)
            ...
            for parami = 1:numel(parfitnumbers)
    ...
    if (T>0)&&(T<Tmax)
        ...
        while (N<Numberofparticleseachjob)&&(i<MaxStep)
            ...
            while (parami<nparfit)&&(priorOK)  %Only if priorOK is not 0 and parami<nparfit the while loop will stop
                
                parami = parami+1; %update loop variable
...
%ABC_SMC_Algorithm_OLCM_VTD_landscape_v5_Gauss_AbsDistance.m (c) by Elena
%Camacho Aguilar and Meritxell Saez Cornellana

%ABC_SMC_Algorithm_OLCM_VTD_landscape_v5_Gauss_AbsDistance.m is licensed under a
